...
- [x] 向量数据库对话历史存储与检索（已集成LLM embedding + MySQL向量存储 + 余弦相似度检索）
...
## 模块8：长期记忆
- [x] 向量数据库集成（已实现LLM embedding + MySQL向量存储方案）
- [x] 对话历史embedding存储（数据库表已设计）
- [x] 用户偏好记录与检索（数据库表已设计）
- [x] 个性化回应生成（已集成向量检索注入LLM prompt）
...
- [x] 集成向量数据库实现长期记忆：已实现LLM embedding + MySQL向量存储 + 余弦相似度检索 + 自动记忆提取 + 个性化回应
...
## 向量数据库长期记忆集成（LLM Embedding + MySQL）
- [x] 后端：创建向量记忆服务模块（LLM embedding生成 + MySQL存储 + 余弦相似度检索）
- [x] 后端：实现对话消息自动embedding化并存入向量数据库
- [x] 后端：实现用户偏好自动提取和存储（LLM驱动的记忆提取）
- [x] 后端：将向量记忆检索集成到LLM对话流程（sendMessage + chatWithVoice）
- [x] 后端：创建tRPC路由管理记忆（list/search/add/delete/clear/stats）
- [x] 前端：在对话页面展示记忆检索状态
- [x] 前端：添加记忆管理界面（AvatarMemory页面 - 查看/搜索/添加/删除记忆）
- [x] 编写向量记忆集成相关单元测试（135个测试全部通过）

## GLB模型S3缓存机制
- [x] 后端：实现缓存键生成（基于参数指纹的确定性hash）
- [x] 后端：模型生成前检查S3缓存，命中则直接返回URL
- [x] 后端：模型生成后自动上传GLB到S3并记录缓存
- [x] 后端：实现缓存失效策略（30天TTL过期 + 手动清除 + 按数字人/类型失效）
...
## 3D模型显示与交互优化
- [ ] 调整3D相机视角和缩放，使模型居中显示
- [ ] 实现骨架参数实时驱动模型变形（BlendShapes前端即时预览）
- [ ] 添加模型加载进度条（生成等待期间显示动画）
- [ ] 添加骨骼线框可视化（可切换显示/隐藏）

## 外观定制模块修复
- [ ] 修复肤色调整功能（实时修改3D模型材质颜色）
- [ ] 修复五官参数调整功能（通过morph targets驱动面部变形）
- [ ] 修复妆容参数调整功能（通过材质/纹理叠加实现）
- [ ] 修复性别特征参数调整功能

## 头发3D模型系统
- [ ] 实现程序化头发模型生成（基于Babylon.js粒子/线条系统）
- [ ] 支持发型选择（短发/中发/长发/马尾等预设）
- [ ] 支持头发颜色和渐变调节
- [ ] 头发模型与头部骨骼绑定
